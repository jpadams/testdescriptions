...
func (r *Foo) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Foo":
		...
		case "ContainerEcho":
			...
			return (*Foo).ContainerEcho(&parent, stringArg), nil
		case "GrepDir":
			var parent Foo
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Foo).GrepDir(&parent, ctx, directoryArg, pattern)
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("The Foo module's short description.\n").
			WithObject(
				dag.TypeDef().WithObject("Foo", TypeDefWithObjectOpts{Description: "Functions for working with Foo."}).
					WithFunction(
